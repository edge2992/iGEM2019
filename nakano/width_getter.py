...
def check_overlap(thined_img, coordinate, radius):
    ...
    if coordinate[1]-radius < 0 or coordinate[1]+radius > height or coordinate[0]-radius < 0 or coordinate[0]+radius > width:
        return True
    mask = make_mask(radius)
    clipping = thined_img[coordinate[1]-radius : coordinate[1]+radius+1, coordinate[0]-radius : coordinate[0]+radius+1]
            ...
            if last_intersection != x - 1:
                ...
                while continue_flag and radius < width:
                    ...
                    if check_overlap(thined_img, [x, coordinate[1]], radius):
                        ...
                        if debug: ax.add_artist(plt.Circle((x, coordinate[1]), radius, facecolor='None', edgecolor='red', linewidth=0.3, alpha=0.7))
            last_intersection = x
    # x軸と平行な直線(列)について
    last_intersection = 0
            ...
            if last_intersection != y - 1:
                ...
                while continue_flag and radius < height:
                    ...
                    if check_overlap(thined_img, [coordinate[0], y], radius):
                        continue_flag = False
                        result.append(radius)
                        if debug: ax.add_artist(plt.Circle((coordinate[0], y), radius, facecolor='None', edgecolor='yellow', linewidth=0.3, alpha=0.7))
            last_intersection = y
    if debug: plt.title("get_radius_list")
...
def get_width(thined_img, scan_line_pairs=1, *, debug=False):
    ...
    for i in range(scan_line_pairs):
        radius_list += get_radius_list(thined_img, [int(width / (scan_line_pairs + 1) * (i + 1)), int(height / (scan_line_pairs + 1) * (i + 1))], debug=debug)
    return collections.Counter(radius_list).most_common()[0][0]

if __name__ == '__main__':
    img = image_processing.load_image_grayscale("./features_detection/fingerprint.png")
