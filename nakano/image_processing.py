...
# 返り値: (なし)
def image_save(img, title="", img_path="./image-"+str(int(datetime.now().timestamp()*(10**3)))+".png", dpi=1000):
    plt.imshow(img)
    plt.title(title)
...
def load_image_grayscale(img_path, *, debug=False):
    if debug: print("start: nakano.image_processing.load_image")
    img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
    if debug: image_show(img, "loaded_image")
    if debug: print("end  : nakano.image_processing.load_image")
    return img
...
def blur(img, *, debug=False):
    if debug: print("start: nakano.image_processing.blur")
    result = cv2.medianBlur(img, 5)
    result = cv2.GaussianBlur(result, (5, 5), 0)
    if debug: image_show(img, "blur - before")
    if debug: image_show(result, "blur - after")
    if debug: print("end  : nakano.image_processing.blur")
    return result
...
def threshold(img, *, debug=False):
    if debug: print("start: nakano.image_processing.threshold")
    result = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)
    if debug: image_show(img, "threshold - before")
    if debug: image_show(result, "threshold - after")
    if debug: print("end  : nakano.image_processing.threshold")
    return result
...
def bitwise_not(img, *, debug=False):
    if debug: print("start: nakano.image_processing.bitwise_not")
    result = cv2.bitwise_not(img)
    if debug: image_show(img, "bitwise_not - before")
    if debug: image_show(result, "bitwise_not - after")
    if debug: print("end  : nakano.image_processing.bitwise_not")
    return result

# ノイズ処理(モルフォロジー変換)(二値化済み)
# img: 2次元配列
# 返り値: 2次元配列
def morphological_transformations (img, *, debug=False):
    if debug: print("start: nakano.image_processing.morphological_transformations")
    kernel = np.ones((3, 3), np.uint8)
    result = cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel)
    result = cv2.erode(result, kernel, iterations=1)
    if debug: image_show(img, "morphological_transformations - before")
    if debug: image_show(result, "morphological_transformations - after")
    if debug: print("end  : nakano.image_processing.morphological_transformations")
    return result
...
def thinning(img, *, debug=False):
    if debug: print("start: nakano.image_processing.thinning")
    height, width = img.shape
    result = np.zeros((height, width), np.uint8)
    result = cv2.ximgproc.thinning(img,result,cv2.ximgproc.THINNING_GUOHALL)
    if debug: image_show(img, "thinning - before")
    if debug: image_show(result, "thinning - after")
    if debug: print("end  : nakano.image_processing.thinning")
    return result
...
def generate_features_map(img, *, debug=False):
    ...
    for y in range(1, height - 2):
        for x in range(1, width - 2):
            if img[y][x] == 255:
                neighbour_point = img[y-1][x-1] + img[y][x-1] + img[y+1][x-1] +img[y-1][x] + img[y+1][x] + img[y-1][x+1] + img[y][x+1] + img[y+1][x+1]
    ...
    for y in range(1, height - 2):
        for x in range(1, width - 2):
            if neighbour_point_map[y][x] == 255:
                result[y][x] = 1
                if debug: plt.scatter(x, y, marker='.', linewidths=0.3, facecolor='None', edgecolors='yellow', alpha='0.5')
            if neighbour_point_map[y][x] == 765:
                if not ( neighbour_point_map[y-1][x-1] >= 765 or neighbour_point_map[y][x-1] >= 765 or neighbour_point_map[y+1][x-1] >= 765 or neighbour_point_map[y+1][x] >= 765 ):
                    result[y][x] = 10
                    if debug: plt.scatter(x, y, marker='.', linewidths=0.3,  facecolor='None', edgecolors='red', alpha='0.5')
    
    if debug: plt.title("features")
    if debug: plt.savefig("./image-"+str(int(datetime.now().timestamp()*(10**3)))+".png", dpi=1000)
